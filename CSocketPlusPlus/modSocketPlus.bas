Attribute VB_Name = "modSocketPlus"
'**************************************************************************************
'
'modSocketPlus module 1.2
'Copyright (c) 2004 by Emiliano Scavuzzo <anshoku@yahoo.com>
'
'Rosario, Argentina
'
'**************************************************************************************
'This module contains API declarations and helper functions for the CSocketPlus class
'**************************************************************************************
...
'==============================================================================
'SOCKET ARRAY ERROR CODES
'==============================================================================
Public Const sckWrongIndex      As Long = 40027
Public Const sckUsedIndex       As Long = 40028
Public Const sckMissingIndex    As Long = 40029
...
If Not m_blnInitiated Then
    ...
    Else
        ...
        ErrRaise lngResult, "modSocketPlus.InitiateProcesses", GetErrorDescription(lngResult)
        Exit Function
    End If
...
If m_blnInitiated And m_lngSocksQuantity = 0 Then
    If FinalizeService = SOCKET_ERROR Then
        ...
        ErrRaise lngErrorCode, "modSocketPlus.FinalizeProcesses", GetErrorDescription(lngErrorCode)
        Exit Function
    Else
...
If lngResult = 0 Then
    ...
    ErrRaise sckOutOfMemory, "modSocketPlus.DestroyWinsockMessageWindow", "Out of memory"
    Exit Function
Else
...
If m_colSocketsInst Is Nothing Then
    ...
    Debug.Print "OK Created socket array"
    
    If CreateWinsockMessageWindow <> 0 Then
        ErrRaise sckOutOfMemory, "modSocketPlus.RegisterSocket", "Out of memory"
        Exit Function
    End If
...
If blnEvents Then
    ...
    If lngResult = SOCKET_ERROR Then
        ...
        lngErrorCode = Err.LastDllError
        ErrRaise lngErrorCode, "modSocketPlus.RegisterSocket", GetErrorDescription(lngErrorCode)
        Exit Function
    Else
...
If m_colSocketsInst.Count = 0 Then
    ...
    Debug.Print "OK Destroyed socket array"
End If
End Sub
...
'Assing a temporal instance of CSocketPlus to a
...
End If
Dim Socket As CSocketPlus
Set Socket = New CSocketPlus
Socket.ArrayAdd "Temporal_Socket"
Socket.Accept "Temporal_Socket", lngSocket
...
Public Function GetAcceptClass(ByVal lngSocket As Long) As CSocketPlus
On Error Resume Next
...
Public Sub ErrRaise(ByVal Number As Long, Optional ByRef Source As String, Optional ByRef Description As String)
On Error Resume Next
