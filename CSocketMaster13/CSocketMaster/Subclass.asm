...
%define _patch4_    04BCCAABh           ;Relative address of WSACleanup
%define _patch5_    05BCCAABh           ;Control timer handle
%define _patch6_    06BCCAABh           ;Relative address of KillTimer
%define _patch7_    07BCCAABh           ;Address of the previous WndProc
%define _patch8_    08BCCAABh           ;Relative address of CallWindowProc
%define _patch9_    09BCCAABh           ;TableA entry count
%define _patchA_    0ABCCAABh           ;TableA1
%define _patchB_    0BBCCAABh           ;TableA2
%define _patchC_    0CBCCAABh           ;TableB entry count
%define _patchD_    0DBCCAABh           ;TableB1
%define _patchE_    0EBCCAABh           ;TableB2
...
%define RESOLVE_MESSAGE 8000h
%define SOCKET_MESSAGE  8001h
...
_no_ide:
    ...
    call    _original                   ;message wasn't W_T neither R_M neither S_M so we call original WndProc
...
_unsub:     ;IDE has stopped, unsubclass the window
    ...
    db      0e8h                        ;Far call
    dd      _patch4_			;Relative address of WSACleanup, patched at runtime
    push    _patch5_                    ;Control timer handle, patched at runtime
    push    0                           ;hWnd, we didn't use it
    db      0e8h                        ;Far call
    dd      _patch6_			;Relative address of KillTimer, patched at runtime
    jmp     _return                     ;Return
...
_original:  ;Call original WndProc
    ...
    push    _patch7_                    ;Address of the previous WndProc, patched at runtime
    db      0e8h                        ;Far Call 
    dd      _patch8_                    ;Relative address of CallWindowProc, patched at runtime
    ret
    

_find_owner_RM:
    mov     ebx, _patch9_               ;TableA entry count
    mov     eax, wParam                 ;Async handle
    mov     edi, _patchA_               ;TableA1
    ...
    dec     ebx                         ;ebx = ebx - 1
    mov     ebx, [ebx*4+_patchB_]       ;Now ebx is a pointer to async owner
    jmp     _call                       ;Prepare to make de callback

_find_owner_SM:
    mov     ebx, _patchC_               ;TableB entry count
    mov     eax, wParam                 ;Socket handle
    mov     edi, _patchD_               ;TableB1
    ...
    sub     ebx, ecx                    ;ebx = ebx - ecx
    dec     ebx                         ;ebx = ebx - 1
    mov     ebx, [ebx*4+_patchE_]       ;Now ebx is a pointer to socket owner
...
_call:      ;Callback to the owners
    ...
    mov     ebx, [ebx+1Ch]              ;Get the address of VB WndProc into ebx
    mov     eax, ebx                    ;Copy the address of VB WndProc into eax
